package de.naoth.xabsleditor.autocomplete;
...
//}
	...
	protected String getXmlResource() {
            return this.getClass().getResource("c.xml").toString();
	}
	...
	protected void loadCodeCompletionsFromXml(DefaultCompletionProvider cp) {
		...
		if (res!=null) { // Subclasses may specify a null value
			InputStream in = this.getClass().getResourceAsStream("c.xml");//cl.getResourceAsStream(res);
			try {
				if (in!=null) {
					cp.loadFromXML(in);
					in.close();
    ...
    /**
     * The provider to use localy in xabsl options (e.g. after goto)
     */
    private CompletionProvider xabslLocalCompletionProvider;

    public CompletionProvider getXabslLocalCompletionProvider() {
        return xabslLocalCompletionProvider;
    }

    public void setXabslLocalCompletionProvider(CompletionProvider xabslLocalCompletionProvider) {
        this.xabslLocalCompletionProvider = xabslLocalCompletionProvider;
    }
